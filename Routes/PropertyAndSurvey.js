...
router.get('/GetPropertyAndSurveySites', function (req, res) {

    var request = new sql.Request();

    request.execute("spGetPropertiesAndSurveySites", function (error, results, feilds) {
        if (error) {
...
router.get('/GetPropertyAndSurveyBuildings', function (req, res) {

    var request = new sql.Request();
    request.input("SiteNumber", sql.Int, req.query.SiteNumber)
    request.execute("spGetPropertyAndSurveyBuildings", function (error, results, feilds) {
        if (error) {
            res.json({ "success": false, "error": error });
        }
        else {
